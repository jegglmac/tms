...
import ShipmentTracking from './ShipmentTrackingWorking';
        ...
        <>
          ...
          <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-4">
            {stats.map((item) => (
              <div
                key={item.name}
                className="relative bg-white pt-5 px-4 pb-16 sm:pt-6 sm:px-5 lg:px-6 shadow rounded-lg hover:shadow-md transition-shadow cursor-pointer min-h-[140px]"
              ...
              >
                ...
                <dd className="ml-16 mt-1 pb-2 sm:pb-3">
                  <p className="text-lg sm:text-xl lg:text-2xl font-semibold text-gray-900">{item.stat}</p>
                  {item.change && (
                    <p
                      className={classNames(
                        item.changeType === 'increase' ? 'text-green-600' : 'text-red-600',
                        'flex items-center text-xs sm:text-sm font-semibold mt-1'
                      )}
                    >
                      {item.changeType === 'increase' ? (
                        <ArrowUpIcon className="self-center flex-shrink-0 h-3 w-3 text-green-500 mr-1" aria-hidden="true" />
                      ) : (
                        <ArrowDownIcon className="self-center flex-shrink-0 h-3 w-3 text-red-500 mr-1" aria-hidden="true" />
                      )}
                      <span className="sr-only"> {item.changeType === 'increase' ? 'Increased' : 'Decreased'} by </span>
                      <span>{item.change}</span>
                    </p>
                  )}
                  <div className="absolute bottom-0 inset-x-0 bg-gray-50 px-4 py-3 sm:px-5">
                    <div className="text-sm flex items-center">
                      <span className="text-gray-600 mr-1">from</span>
                      <span className="font-medium text-gray-900 truncate">{item.previousStat}</span>
                    </div>
                  </div>
                </dd>
              </div>
